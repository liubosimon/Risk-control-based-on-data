...
train = pd.read_csv('../data1/8file_train_sample.csv')
train_y = train.target
train_x = train.drop('target',axis=1)

test = pd.read_csv('../data1/8file_test.csv')
test_Idx = test.Idx
test_x = test.drop('Idx',axis=1)

all_features =  list(train_x.columns)
features_1 = list(pd.read_csv('../feature_select/feature_score.csv')['feature'])
features_2 = [feature for feature in all_features if feature not in features_1]
